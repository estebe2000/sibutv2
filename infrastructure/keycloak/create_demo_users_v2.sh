...
upsert_user() {
    ...
    USER_ID=$(curl -s -H "Authorization: Bearer $TOKEN" "$HOST/admin/realms/$REALM/users?username=$USERNAME" | grep -o '"id":"[^" ]*"' | head -1 | cut -d '"' -f 4)
...
curl -s -X POST "$HOST/admin/realms/$REALM/roles" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"name": "ADMIN"}'
curl -s -X POST "$HOST/admin/realms/$REALM/roles" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"name": "PROFESSOR"}'
curl -s -X POST "$HOST/admin/realms/$REALM/roles" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"name": "STUDENT"}'

# Upsert Users
upsert_user "superprof" "superprof" "ADMIN"
upsert_user "prof" "prof" "PROFESSOR"
upsert_user "tc-demo" "tc" "STUDENT"
