...
class Tools:
    def __init__(self):
        # Paramètres fixes du Docker-compose pour but_tc_db
        self.db_host = "host.docker.internal" 
        self.db_port = "5432"
        self.db_user = "skills_user" # Based on usual env
        self.db_pass = "skills_password"
        self.db_name = "skills_hub"
    ...
    def get_course_details(self, code: str) -> str:
        """
        Trouve les détails complets d'un cours (Ressource) ou d'une activité (SAÉ) à partir de son code.
        :param code: Le code officiel du cours ou de l'activité (ex: "R1.13", "SAE 4.01", "R2.10").
        :return: Une description textuelle complète de l'élément pédagogique.
        """
        conn = self._get_connection()
        if not conn:
            return "Erreur: Impossible de se connecter à la base de données des cours."
...
{desc}"
            ...
            cur.close()
            conn.close()
            return f"Aucune information trouvée en base pour le code {code}."
