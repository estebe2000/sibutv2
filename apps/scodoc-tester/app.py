...
<body>
    <div class="card">
        <h1>🔍 ScoDoc Tester</h1>
        <form method="POST">
            <label>URL ScoDoc (Web)</label>
            <input type="text" name="url" value="https://scodoc.univ-lehavre.fr" required>
            
            <label>URL API (Optionnel, ex: https://scodoc.univ-lehavre.fr/ScoDoc/api)</label>
            <input type="text" name="api_url" placeholder="Laisser vide pour utiliser URL Web + /ScoDoc/api">
...
def home():
    ...
    if request.method == "POST":
        ...
        custom_api_url = request.form.get("api_url", "").strip().rstrip("/")
        ...
        if custom_api_url:
            api_token_url = f"{custom_api_url}/tokens"
        else:
            api_token_url = f"{base_url}/ScoDoc/api/tokens"
        ...
        try:
            ...
            if not csrf:
                ...
                if "Déconnexion" in r2.text or "Scolarité" in r2.text:
                    result_html = f'<div class="result success">✅ <b>CONNEXION WEB RÉUSSIE !</b><br>Session active.</div>'
                    
                    # Test API
                    result_html += f"<br>🔍 Test API sur : {api_token_url}<br>"
                    api_res = session.post(api_token_url, json={"username": username, "password": password})
                    
                    if api_res.status_code == 200:
                        token_val = api_res.json().get('token', 'N/A')
                        result_html += f'<div class="result success">✅ API ACCESSIBLE (Token OK)<br>Token: {token_val[:15]}...</div>'
                    else:
                        result_html += f'<div class="result error">⚠️ API REFUSÉE (Code {api_res.status_code})<br>Réponse : {api_res.text[:200]}</div>'
                else:
                    result_html = f'<div class="result error">❌ ÉCHEC LOGIN WEB (Pas de redirection/session).</div>'
