...
export function ProfessorDashboard({ user, curriculum, setActiveTab }: any) {
  ...
  return (
    <Container size="xl" py="xl">
        ...
        <Grid>
            {/* Colonne de droite (Tutorat) remonte en premier sur mobile */}
            ...
            {/* Colonne de gauche (Activités) passe en second sur mobile */}
                ...
                {myActivities.length === 0 ? (
                    ...
                    <Stack gap="md">
                        {myActivities.map((act: any) => (
                            <Paper key={act.id} withBorder p={isMobile ? "lg" : "md"} radius="md" shadow="xs" style={{ borderLeft: '4px solid #fd7e14' }}>
                                <Stack gap="xs">
                                    <Group justify="space-between">
                                        <Group gap="xs">
                                            <Badge color="orange" variant="light">{act.code}</Badge>
                                            <Text fw={600} size={isMobile ? "md" : "sm"}>{act.label}</Text>
                                        </Group>
                                        <Badge size="xs" color="gray" variant="outline">Sem. {act.semester}</Badge>
                                    </Group>
                                    
                                    <Text size="sm" lineClamp={2} c="dimmed">{act.description}</Text>
