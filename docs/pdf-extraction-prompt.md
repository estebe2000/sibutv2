...
### B. Activités (SAÉ)
- **Codes** : `SAÉ X.XX` (ex: `SAE 1.01`).
- **Champs requis** : 
  - `code` : "SAE 1.01"
  - `nom` : Titre exact (ex: "Marketing : positionnement...")
  - `description` : Concaténer "Objectifs et problématique professionnelle" + "Descriptif générique". Garder les sauts de ligne.
...
- **Champs requis** : 
  ...
  - `content` : "Contenu" complet (liste des savoirs).
  - `keywords` : "Mots clés". (Ajouter à la fin de la description ou stocker à part).
  - `hours` : Volume horaire défini nationalement (entier).
...
## 4. Tâche Spécifique
Extrais les SAÉ et Ressources du **Semestre [X]** (ou Niveau [X]).